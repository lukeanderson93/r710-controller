...
class Server():
    ...
    def power_on(self, fan_speed_pct=None):
        ...
        if fan_speed_pct is not None:
            if not (fan_speed_pct >= 1 and fan_speed_pct <= 100):
                raise ValueError('The fanspeed pct should be an integer between 1-100.')

            time.sleep(2)
            self.set_fan_speed_manual(fan_speed_pct=int(fan_speed_pct))
        return out
    ...
    def power_off_soft(self):
        ...
        Returns: the response (if any) from the server
        """
        print(f"[IPMI] - ({self.__host}) - Executing graceful shutdown.")
